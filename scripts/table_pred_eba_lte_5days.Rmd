...
spr_after <- spr_after %>% filter(days_past_onset<6|is.na(days_past_onset))
...
table_1_all <- read.csv(file="../output/si_tab2_demo_lte_5days.csv")
...
keep: ~variant+age_entry+sex_birth
```{r eval=FALSE, include=FALSE}
vars <- c('log_mt', 'log_sal',  'cough_count','lr', 'gi', 'sys')
...
lm <- lmer (yL_coarse ~ (1|subject_id) + (1|subject_id:sample_date) +age_entry+sex_birth+log_sal+cough_count+sys+relevel(as.factor(variant), ref = "Other"), data=coarse)
x_matrix <- getME(lm, "X")

#Run the LEMC()
fit_ML = lmec(yL=yL_coarse, cens=cens_coarse, X=x_matrix, Z=Z_coarse, cluster=cluster_coarse, maxstep = 200, varstruct = "unstructured", method = "ML")

vars <- c("intercept", "age_entry","sex_birth","log_sal", "cough_count","sys","Alpha", "Delta", "Omicron" )
for (i in c(2:length(vars))){
                est <- fit_ML$beta[i]
                se <- sqrt(diag(fit_ML$varFix))[i]
                up <- est+1.96*se
...
write.csv(table4output1,file="../output/si_tab4b_pred_eba_lte_5days.csv",row.names=F)
```
