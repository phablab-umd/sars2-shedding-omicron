...
#check if the sero_collection_date_1 is near the first shedding visit sample date: range 0-3
fine <- fine %>% mutate(eba_sero_diff = as.Date(sample_date) - as.Date(sero_collection_date_1))
range((fine %>% filter(!is.na(eba_sero_diff)))$eba_sero_diff)
...
#age, log_mt, log_sal, cough_count, sys
```
...
```{r eval=FALSE, include=FALSE}
vars <- c( 'log_mt', 'log_sal', 'cough_count', 'sys')
...
aic_final #279.834
model_final #"BA2+age_entry+sex_birth+log_sal+cough_count+sys"
```

Multivariable analysis for Fine aerosol
```{r}
est_ci_fine_multi <- c()
lm <- lmer (yL_fine ~ (1|subject_id) + (1|subject_id:sample_date) + BA2+age_entry+sex_birth+log_sal+cough_count+sys , data=fine)
x_matrix <- getME(lm, "X")

#Run the LEMC()
fit_ML = lmec(yL=yL_fine, cens=cens_fine, X=x_matrix, Z=Z_fine, cluster=cluster_fine, maxstep = 200, varstruct = "unstructured", method = "ML")

vars <- c('intercept', 'BA2',"age_entry", "sex_birth","log_sal", "cough_count", "sys")
...
vars_coarse <- c( 'BA2', 'days_since_vax', 'age_entry','sex_birth','days_past_onset', 'log_mt', 'log_sal', 'cough_count', 'ur', 'lr', 'gi', 'sys')
...
#age_entry, days_past_onset, log_mt, log_sal, gi, sys
```
##model selection based on AIC: 
...
#BA2+age_entry+sex_birth+boosted+days_past_onset+log_sal+gi+sys 264.7514
#"BA2+age_entry+sex_birth+log_sal+sys" 302.567
```

Multivariable analysis for coarse aerosol
```{r}
est_ci_coarse_multi <- c()
lm <- lmer (yL_coarse ~ (1|subject_id) + (1|subject_id:sample_date) + BA2+age_entry+sex_birth+log_sal+sys, data=coarse)
x_matrix <- getME(lm, "X")

#Run the LEMC()
fit_ML = lmec(yL=yL_coarse, cens=cens_coarse, X=x_matrix, Z=Z_coarse, cluster=cluster_coarse, maxstep = 400, varstruct = "unstructured", method = "ML") #maxstep=200 didn't converge, so increase the value to 400

vars <- c('intercept', 'BA2',"age_entry","sex_birth", "log_sal","sys")
for (i in c(2:length(vars))){
                est <- fit_ML$beta[i]
                se <- sqrt(diag(fit_ML$varFix))[i]
                up <- est+1.96*se
...
ll<-fit_ML$loglik
aic <- -2*ll+2*(str_count("BA2+age_entry+sex_birth+log_sal+sys",coll("+"))+2+6)
aic #302.5068
```
...
lm <- lmer (log(sample_quant+1) ~ (1|subject_id)+(1|subject_id:sample_date) + boosted, data=coarse)
...
vars <- c('intercept', 'BA2',"age_entry","sex_birth", 'boosted','days_past_onset',"log_sal",'gi',"sys","anti_nuc","boosted*anti_nuc")
for (i in c(2:length(vars))){
                est <- fit_ML$beta[i]
                se <- sqrt(diag(fit_ML$varFix))[i]
                up <- est+1.96*se
...
table3output <- meanCIlist[["fine_unadj"]] %>% select(varname,predict1) %>% rename(`Fine Unadjusted`=predict1)
table3output <- table3output %>% left_join(meanCIlist[["fine_adj"]] %>% select(varname,predict1),by="varname") %>% rename(`Fine Adjusted`=predict1)
table3output <- table3output %>% left_join(meanCIlist[["coarse_unadj"]] %>% select(varname,predict1),by="varname") %>% rename(`Coarse Unadjusted`=predict1)
...
tableorder <- c('BA2','boosted','anti_nuc','days_since_vax', 'age_entry','sex_birth','days_past_onset', 'log_mt', 'log_sal', 'cough_count', 'ur', 'lr', 'gi', 'sys')
...
write.csv(table3output1,file="../output/table_model_omicron_v2.csv",row.names=F)
```
